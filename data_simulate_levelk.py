...
Choose the desired level-k by setting k variable.
Use this for only Simulation.
This code is almost the same as the original dynamic simulation, but it uses the same level-k model for all three options.
...
ORIGINAL_DATA_PATH = BASE_DIR / "ATR_Original" / "experiments" / "some_title" / "dynamic" / "training" / "models"
...
STUDY_TITLE = f"study_data_level{k}" # you can change this.
...
def update_motion_old(ego_df, frame_id,act):
    ...
    return reached_end, ego_df, merged
...
def select_path(is_simulation=False):
    ...
    STUDY_RES_DIR = DATA_SIMULATION_PATH if is_simulation else TRAINING_HISTORY_PATH
...
def load_dynamic_model(is_simulation=False):
    ...
    for i in range(1, 4):
        agent_levelk_paths[i] = Path(EXPERIMENTS_PATH) / "some_title" / f"level{k}" / \
...
def run_episode(ego_log, total_timesteps, final_state_df, episode_no, run_no, ego_count, epoch_no, scenario_id, o, save_dir=""):
    ...
    for frame_id in frames:
        ...
        save_rewards([reward], file_path=f"rewards_scenario.csv", reset=False)
        total_timesteps += 1
    ...
    if not collided:
        ...
        save_point_distances([frechet_distance], file_path=f"training_frechet_scenario{scenario_id}.csv", reset=False)
        
        # Save the new distance (appends by default if reset=False)
        save_point_distances([distance], file_path="training_frechet_distances.csv", reset=False)
    print(f"""
    =====================================================
    ** EGO_ID: {EGO_ID} | Run No: {run_no} | Total Episode No: {episode_no} **
    *****************************************************
    ...
    boltzmann_decay_end_state = NUM_EPOCHS*12*5/3 # multiply it by episode numbers per num epochs 
    ...
    save_path = STUDY_RES_DIR / save_dir / f"{scenario_id}-{o}.csv"
...
def simulate_episode(ego_log, total_timesteps, final_state_df, episode_no, run_no, ego_count, epoch_no, scenario_id, o, save_dir=""):
    ...
    for frame_id in frames:
        ...
        dynamic_actionget = ego_info[0][2]  # from 0 up to 3
        
        action_type = ego_info[1][0]  # ['Random-Argmax', 2] for low level
        actionget = ego_info[1][dynamic_actionget][2]  # Extracted action (e.g., 2) low level
...
def main():
    ...
    if Train:
        for run_no, scenario_id in enumerate(scenario_id_list):
            # save_point_distances([], file_path="training_frechet_distances.csv", reset=True)
            save_point_distances([], file_path=f"{FRECHET_TRAINING_PATH}/training_frechet_scenario{scenario_id}.csv", reset=True)
    elif Simulate:
        for run_no, scenario_id in enumerate(scenario_id_list):
            # save_point_distances([], file_path="simulation_frechet_distances.csv", reset=True)
