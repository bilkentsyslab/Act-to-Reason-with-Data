...
class DynamicDQNAgent:
    ...
    def _build_model(self):
        model = Sequential()
        model.add(Flatten(input_shape=(1, self.dynamic_state_size)))  # Input layer
        ...
        model.compile(loss='mse', optimizer=Adam(learning_rate=self.learning_rate, clipnorm=5.0))
        return model
    ...
    def _load_levelk(self,path,random):
        ...
        agentlevk.T = 1 #Sets the boltzmann temp. of Level-k cars to 1, prevents extra-random actions #BURASI
        agentlevk.boltzmann_sampling = True # random
        return agentlevk
    ...
    def act(self, state, remove_merging=False, get_qvals=False):
        if self.random_dynamic_strategy:
            random_strategy = random.randint(0, 8) #randomly selects a level-k strategy
            ...
            if get_qvals:
                ...
                return [["Random-Strategy "+ str(random_strategy), random_strategy], self.agentlevk[random_strategy//3].act(
            ...
            if get_qvals:  # Return Q values with action-related information
                ...
                for i, cum_prob in enumerate(cumulative_prob):
                    if self.boltzmann_sampling and (rand < cum_prob):
                        if i != 9:
                            if greedy == i:
                                return [["Random-Argmax",q_values.tolist(),  greedy], self.agentlevk[greedy//3].act(  # greedy//3 is the level-k agent
                                    state=state[:, :, 0:self.state_size], remove_merging=True), levk_qvals]
                            else:
                                return [["Random",q_values.tolist(), i], self.agentlevk[i//3].act(
                                    state=state[:, :, 0:self.state_size], remove_merging=True), levk_qvals]
                        else:
                            if greedy == i:
                                return [["Merge-Argmax",q_values.tolist(),  greedy], ["Merge", 5], levk_qvals]

                            else:
                                return [["Merge-Random",q_values.tolist(),  i], ["Merge", 5], levk_qvals]


                # If none of the above, return the greedy action
                return [["Argmax",q_values.tolist(), greedy], self.agentlevk[greedy//3].act(
                    state=state[:, :, 0:self.state_size]), levk_qvals]
    ...
    def compute_q_loss(self, predicted_q_values, target_q_values):
        return np.mean((predicted_q_values - target_q_values) ** 2)
    ...
    # Load the old model with 3 outputs
        old_model = load_model(fname)
        old_weights = old_model.get_weights()

        # Build a new model with expanded output neurons (9 outputs)
        self.model = self._build_model()
        new_weights = self.model.get_weights()

        # Transfer old weights (excluding the output layer)
        for i in range(len(old_weights) - 2):  # Ignore last 2 layers (output weights & bias)
            new_weights[i] = old_weights[i]

        # Initialize new output neurons randomly
        num_old_outputs = old_weights[-2].shape[1]  # 3 old outputs
        num_new_outputs = new_weights[-2].shape[1]  # 10 new outputs

        new_output_weights = np.random.normal(scale=0.001, size=(new_weights[-2].shape[0], num_new_outputs))
        new_output_bias = np.random.normal(scale=0.001, size=(num_new_outputs,))

        scaling_factor = 1
        for old_idx in range(num_old_outputs):
            new_output_weights[:, old_idx * 3 : (old_idx + 1) * 3] = scaling_factor * np.tile(old_weights[-2][:, old_idx:old_idx + 1], (1, 3))
            new_output_bias[old_idx * 3 : (old_idx + 1) * 3] = scaling_factor * np.tile(old_weights[-1][old_idx], 3)



        # Compute the average Q-value from old output layer
        avg_q_weights = np.mean(old_weights[-2], axis=1)  # Average weight per input neuron
        avg_q_bias = np.mean(old_weights[-1])  # Average bias across old outputs

        # Set the weight and bias for the new 10th output node using the average Q-value
        new_output_weights[:, -1] = avg_q_weights * scaling_factor  # Assign average weights
        new_output_bias[-1] = avg_q_bias * scaling_factor  # Assign average bias

        # Set updated weights
        new_weights[-2] = new_output_weights
        new_weights[-1] = new_output_bias
        self.model.set_weights(new_weights)
        dummy_input = np.ones((1, 1, 99))  # Shape is (1, 1, 99)
        output_values = self.model.predict(dummy_input)
